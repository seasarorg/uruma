...
/**
 * セレクション・バインディングと画面定義の整合性を確認するHack
 * 
 * @author  snuffkin
 */
public class SelectionBindingValidator {

    /**
	 * インポート・セレクション・バインディングに対応する画面定義の存在を確認するHack　Action
	 * @when action.simpleName =~ ".*Action" field.parent = action annotation.type <=  {@link ImportValue} annotation in field.annotations
	 */
	public void validateImportSelectionBinding(CtClass<?> action,
			CtField<?> field, CtAnnotationInstance<?> annotation,
			Messager messager) {
		HackUtil.validateField(action, field, annotation, messager);
	}

	/**
	 * エクスポート・セレクション・バインディングに対応する画面定義の存在を確認するHack　Action
	 * @when action.simpleName =~ ".*Action" field.parent = action annotation.type <=  {@link ExportValue} annotation in field.annotations
	 */
	public void validateExportSelectionBinding(CtClass<?> action,
			CtField<?> field, CtAnnotationInstance<?> annotation,
			Messager messager) {
		HackUtil.validateField(action, field, annotation, messager);
	}
